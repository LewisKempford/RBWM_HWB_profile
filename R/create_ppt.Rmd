...
params:
  ...
  further_info: TRUE
  indicator_measures: TRUE
  benchmarking: TRUE
  lsoa_boundaries: TRUE
...
setwd(paste0("C:/Users/", Sys.getenv("username"), "/OneDrive - Royal Borough of Windsor and Maidenhead/projects_git/RBWM_HWB_profile/R"))
...
incl_contact <- report_params$contact

# People and place
incl_title_people_and_place<- report_params$title_people_and_place
incl_population <- report_params$population
incl_ethnicity_language <- report_params$ethnicity_language
incl_imd <- report_params$imd
incl_dep_domains <- report_params$dep_domains
...
# Further information and resources
incl_further_info <- report_params$further_info
incl_indicator_measures <- report_params$indicator_measures
incl_benchmarking <- report_params$benchmarking
incl_lsoa_boundaries <- report_params$lsoa_boundaries
incl_msoa_boundaries <- report_params$msoa_boundaries
incl_ward_boundaries <- report_params$ward_boundaries
incl_lsoa_ward_map <- report_params$lsoa_ward_map
incl_accessibility <- report_params$accessibility
incl_references <- report_params$references
incl_resources <- report_params$resources
...
for (i in seq_along(tables)) {
  ...
  my_doc <- ph_with(mydoc, location = location, value = tables[[i]], 
                    alt_text = paste0("Summary table ", i))
}
...
```{r contact, eval = incl_contact}

slide_num <- slide_num + 1

title <- paste0("Contact details")

text <- unordered_list(
  level_list = c(0, 0, 0, 0, 0),
  str_list = c("For questions or additional information about these profiles, please contact:",
               "Senior Public Health Analyst",
               "Public Health",
               "Royal Borough of Windsor and Maidenhead",
               "phi@rbwm.gov.uk") # level 0
  )
...
pop_comp <- plot_pop_area_comp(data = x)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "content right"), 
                 value = pop_comp$plot, alt_text = pop_comp$alt_text, res = 140)

```

```{r ethnicity_language, eval = incl_ethnicity_language}
...
ind_sex <- NULL
ind_age <- NULL
title <- paste0("Ethnicity and main language")
short_title <- NULL
context <- NULL
...
# Outputs
ethnicity_plot <- plot_grouped_bar(data = x, plot_title = "Ethnicity",
                                   x_axis = "Percentage of area's population (%)",
                                   source = "Nomis. 2021 Census.")
...
# Outputs
main_lang_plot <- plot_grouped_bar(data = x, plot_title = "Main language spoken",
                                   x_axis = "Percentage of area's population (%)",
                                   colour_palette = c("#AA9767", "grey"),
                                   source = "Nomis. 2021 Census.")
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "version"), 
                 value = paste0("Version ", format(version_num, nsmall = 1)))
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "content left"), 
                 value = ethnicity_plot$plot, alt_text = ethnicity_plot$alt_text,
                 res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "content right"), 
                 value = main_lang_plot$plot, alt_text = main_lang_plot$alt_text, 
                 res = 140)
...
# Outputs
pop_dep_bar_plot <- plot_dep_pop(data = x, lsoa_codes = rbwm_lsoas)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "version"), 
                 value = paste0("Version ", format(version_num, nsmall = 1)))
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "content left"), 
                 value = pop_dep_bar_plot$plot, 
                 alt_text = pop_dep_bar_plot$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "content right"), 
                 value = dep_map$plot, alt_text = dep_map$alt_text, res = 140)

```

```{r dep_domains, eval = incl_dep_domains}
...
ind_sex <- "persons"
ind_age <- NULL
title <- paste0("Deprivation domains")
short_title <- NULL
context <- NULL
value_type <- NULL
value_type_short <- NULL
...
# plot first eight domains
x <- x[x$GroupingVariable != "Overall deprivation (IMD)", ]
domains_map <- map_deprivation_domains(deprivation_data = x,
                                       lsoa_shape = shp_lsoa,
                                       lsoas_to_plot = rbwm_lsoas)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "version"), 
                 value = paste0("Version ", format(version_num, nsmall = 1)))
mydoc <- ph_with(mydoc, 
                 location = ph_location_label(ph_label = "content"), 
                 value = domains_map$plot, alt_text = domains_map$alt_text, 
                 res = 140)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "latest period"), 
                 value = latest_period, alt_text = latest_period, res = 140)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "compare areas"), 
                 value = area_comp$plot, alt_text = area_comp$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "map"), 
                 value = map$plot, alt_text = map$alt_text, res = 140)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "version"), 
                 value = paste0("Version ", format(version_num, nsmall = 1)))
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "overview"), 
                 value = latest_value$plot, alt_text = latest_value$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "latest period"), 
                 value = latest_period, alt_text = latest_period, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "trend"), 
                 value = trend_plot$plot, alt_text = trend_plot$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "compare areas"), 
                 value = area_comp$plot, alt_text = area_comp$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "map"), 
                 value = map$plot, alt_text = map$alt_text, res = 140)
...
area_comp <- plot_areas_quint(data = x, area_codes = rbwm_msoas,
                              value_type = value_type_short, show_ci = FALSE)
...
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "latest period"), 
                 value = latest_period, alt_text = latest_period, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "trend"), 
                 value = trend_plot, alt_text = trend_plot, res = 140, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "compare areas"), 
                 value = area_comp$plot, alt_text = area_comp$alt_text, res = 140)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "map"), 
                 value = map$plot, alt_text = map$alt_text, res = 140)
...
```{r title_further_info, eval = incl_further_info}

# ---- Further information and resources ----
slide_num <- slide_num + 1

title <- paste0("Further information and resources")
...
```{r indicator_measures, eval = incl_indicator_measures}

slide_num <- slide_num + 1

title <- paste0("Indicator measures")

text <- unordered_list(
  level_list = c(0, 1, 1, 1),
  str_list = c(
    "The indicators in this profile use a variety of measures to present the data effectively, these include:", # level 0
    "Proportion or percentage (%): a way to show a part of a whole amount. For example, if 25 out of 100 people have a certain condition, that would be shown as 25%.", # level 1
    "Crude rate: a measure that shows the total number of events (like deaths, births, or disease cases) in a population over a specific time period. It's usually expressed per thousands of people to allow for easy comparison.", # level 1
    "Indirectly standardised ratio (ISR): a way to compare the rate of an event in a population to what would be expected if that population had the same age-specific rates as a standard population. It adjusts for differences in age structure, making comparisons between populations more meaningful. ISRs are often expressed as a ratio or percentage, where a value of 100 (or 1.0) means the observed rate is equal to the expected rate." # level 1
    ),
    style = list(
    fp_text(font.size = 20),
    fp_text(font.size = 20),
...
```{r benchmarking, eval = incl_benchmarking}

slide_num <- slide_num + 1

title <- paste0("Benchmarking methods")

text <- unordered_list(
  level_list = c(1, 1, 1, 1),
  str_list = c(
    "RAG comparison (Red, Amber, Green) indicates how well an area is performing compared to a benchmark, typically England. The comparison is created by comparing the area's value to a reference range, which is usually based on confidence intervals. Green shows a value that is better, red to a value that is worse and amber indicates that there is no difference when compared to the benchmark.",
    "BOB comparison (Blue, Orange, Blue) is used when a value judgement is not appropriate. Light blue indicators a higher value and dark blue indicates a lower value compared to a benchmark, typically England. Similar to the RAG method, a comparison is created by comparing the area's value to a reference range.",
    "Quintiles are created using a quantiling method and divide data into five equal groups. This creates relative rankings and distribution. Quintiles are often used highlight inequalities between groups or areas. Quintiles are used in this profile when the underlying data is not based on the population, for example households, or survey based data where we do not have access to the weightings",
    "An indicator is shaded grey where it is not possible to create a compariosn. This could be due to an area having no value or values where counts are below 10."
    ),
  style = list(
...
```{r lsoa_boundaries, eval = incl_lsoa_boundaries}

slide_num <- slide_num + 1

title <- paste0("LSOA boundary map")

map <- map_boundary(shape_file = shp_lsoa, areas_to_map = rbwm_lsoas, 
                    include_legend = FALSE)
text <- paste0("There are ", length(rbwm_lsoas), " LSOAs in ", rbwm_name, ".")
...
```{r msoa_boundaries, eval = incl_msoa_boundaries}

slide_num <- slide_num + 1

title <- paste0("MSOA boundary map")

map <- map_boundary(shape_file = shp_msoa, areas_to_map = rbwm_msoas)
...
```{r ward_boundaries, eval = incl_ward_boundaries}

slide_num <- slide_num + 1

title <- paste0("Ward boundary map")

map <- map_boundary(shape_file = shp_ward, areas_to_map = rbwm_wards)
...
```{r lsoa_ward_map, eval = incl_lsoa_ward_map}

slide_num <- slide_num + 1

title <- paste0("LSOA to Ward best fit boundaries")

map <- map_boundaries_overlay(base_shape = shp_ward, 
                              base_areas_to_map = rbwm_wards, 
                              overlay_shape = shp_lsoa_rbwm, 
                              overlay_areas_to_map = rbwm_lsoas)

text <- unordered_list(
  level_list = c(1, 1, 1, 1),
  str_list = c(
    paste0("There are ", length(rbwm_lsoas), " LSOAs in ", rbwm_name, "."), # level 1
    paste0("There are ", length(rbwm_wards), " Wards in ", rbwm_name, "."), # level 1
    paste0("LSOAs do not align exactly with electoral wards in ", rbwm_name, "."), # level 1
    "Best fit lookups are used when assigning LSOAs to wards."
    ),
  style = list(
    fp_text(font.size = 18),
    fp_text(font.size = 18),
...
# Add slide
mydoc <- add_slide(mydoc, layout = "two content left large", 
                   master = "PHI profiles")
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "title"),
                 value = title)
mydoc <- ph_with(mydoc, location = ph_location_label(ph_label = "slide number"), 
...
```{r accessibility, eval = incl_accessibility}
...
text <- unordered_list(
  level_list = c(1, 1, 1, 2, 2, 2, 1, 2, 2, 2, 2),
  str_list = c(
    "All plots include alternative text (alt text) that describes the visual content and includes relevant values where possible.", # level 1
    "To view the alt text: right-click on the image and select 'View Alt Text' to open the Alt Text pane.", # level 1
    "Shapes are used alongside colour on plots to show comparisons, ensuring accessibility for users who may have difficulty seeing different colours. For example, a RAG rating (Red, Amber, Green) is commonly used to show how an area's value compares to a benchmark, such as the England average:", # level 1
    "Green indicates better performance", # level 2
    "Amber indicates similar performance", # level 2
    "Red indicates worse performance", # level 2
    "To support colour accessibility, shapes are also used to represent these ratings:", # level 1
    "Triangle = better performance", # level 2
    "Circle = similar performance", # level 2
    "Triangle pointing downards = worse performance", # level 2
    "Square = no comparison available" # level 2
    ),
  style = list(
    fp_text(font.size = 18),
    fp_text(font.size = 18),
...
```{r references, eval = incl_references}

slide_num <- slide_num + 1

title <- paste0("References")

text <-paste0("The methodology for each indicator is based on published data and existing measures.\nSources of methodology include Office for Health Improvement and Disparities, Office for National Statistics, and NHS Digital.\nSome indicator values may be different from published figures. This may be due to slight differences in numbers extracted or data being grouped into different time periods.\nOffice for National Statistics licensed under the Open Government Licence v.3.0\nContains OS data \uA9 Crown copyright and database right ", 
format(Sys.time(), "%Y"), "\n\uA9 Stadia Maps \uA9 OpenMapTiles \uA9 OpenStreetmap \uA9 Stamen Design")
...
```{r resources, eval = incl_resources}

slide_num <- slide_num + 1

title <- paste0("Resources")


# Create a paragraph with a clickable hyperlink
text <- fpar(
  hyperlink_ftext(
    text = "RBWM JSNA:",
    href = "https://www.rbwm.gov.uk/community-and-living/public-health/joint-strategic-needs-assessment",
    prop = fp_text(color = "blue", underlined = TRUE, font.size = 24)
  ),
  ftext(" RBWM's JSNA provides information used to assess the current and future health and wellbeing needs of the local population. It helps inform decisions about health priorities, service planning, and resource allocation to improve outcomes and reduce inequalities.")
  )
